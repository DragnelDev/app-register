        ...
        <tbody class="divide-y divide-ink-100">
          <tr v-if="loading">
            <td
              ...
              class="table-cell py-8 text-center text-ink-400 dark:text-ink-500"
            >
              Cargando registros…
            </td>
          </tr>
          ...
          <tr
            ...
            v-else
            :key="idx"
            class="hover:bg-ink-50/60 dark:hover:bg-ink-900/80"
            @click="emit('row-click', row)"
          >
            <td v-for="column in columns" :key="column.key" class="table-cell dark:bg-ink-950">
              <slot :name="`cell-${column.key}`" :row="row">
                {{ row[column.key] }}
              </slot>
            </td>
            <td v-if="$slots.actions" class="table-cell text-right dark:bg-ink-950" @click.stop>
