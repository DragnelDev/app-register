      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <ul class="list-inside list-disc text-ink-600">
            <li v-for="p in comprobante.preventivos" :key="p.numeroPreventivo">
              {{ p.numeroPreventivo }}
            </li>
          </ul>
        </div>
        <div>
          ...
          <ul class="list-inside list-disc text-ink-600">
            <li v-for="d in comprobante.devengados" :key="d.numeroDevengado">
              {{ d.numeroDevengado }}
            </li>
          </ul>
        </div>
        <div>
          ...
          <ul class="list-inside list-disc text-ink-600">
            <li v-for="b in comprobante.beneficiarios" :key="b.nombreBeneficiario">
      ...
      <div>
        ...
        <ul class="list-inside list-disc text-ink-600">
          <li v-for="u in comprobante.carpetasUbicacion" :key="`${u.carpetaId}-${u.numeroParte}`">
            Carpeta {{ u.carpeta?.codigoCarpeta ?? u.carpetaId }} — Parte {{ u.numeroParte }} de
            {{ comprobante.cantidadCarpetas }}
          </li>
        </ul>
      </div>
    </div>
