...
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-end justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-ink-600 dark:text-ink-300">
          Resumen del estado actual del archivo documental.
        </p>
      </div>
      <RouterLink
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <div>
          <p class="text-xs font-semibold uppercase tracking-wide text-ink-600 dark:text-ink-300">
            {{ metric.label }}
          </p>
            ...
            <span
              ...
              :class="
                metric.trend.tone === 'warning'
                  ? 'bg-seal-100 text-seal-800 dark:bg-amber-400/15 dark:text-amber-400'
                  : 'bg-emerald-50 text-emerald-800 dark:bg-emerald-400/15 dark:text-emerald-400'
              "
            >
              {{ metric.trend.text }}
            </span>
        ...
        <div
          class="flex h-11 w-11 items-center justify-center rounded-[10px] bg-seal-100 text-xl text-seal-800 dark:bg-seal-400/15 dark:text-seal-300"
          aria-hidden="true"
        >
          {{ metric.icon }}
        </div>
    ...
    <div class="card overflow-hidden">
      ...
      >
        ...
        <RouterLink
          to="/c31"
          class="text-xs font-semibold text-seal-800 hover:underline dark:text-seal-300"
        >
          Ver todos →
        </RouterLink>
      </div>
      ...
      <ul class="divide-y divide-ink-100 dark:divide-white/10">
        <li
          v-for="comprobante in ultimosComprobantes"
          :key="comprobante.id"
        ...
        >
          <div>
            ...
            <p class="text-xs text-ink-600 dark:text-ink-400">
              Bs {{ Number(comprobante.montoTotal).toFixed(2) }} · {{ comprobante.fechaElaboracion }}
            </p>
          </div>
        ...
        <li
          ...
          class="px-5 py-8 text-center text-sm text-ink-600 dark:text-ink-400"
        >
